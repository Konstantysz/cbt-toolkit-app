...
const PAGE_LABELS = [pl.compare.page1, pl.compare.page2, pl.compare.page3, pl.compare.page4];
    ...
    <>
      {emotions.map((em) => (
        <View key={em.name} style={styles.compactEmRow}>
          <Text style={styles.compactEmName}>
            {em.name}
            {em.intensityAfter !== undefined && em.intensityAfter < em.intensityBefore ? ' ↓' : ''}
          </Text>
              ...
              <View style={styles.compactTrack}>
                <View
                  style={[
                    styles.compactFill,
                    {
                      width: `${em.intensityBefore}%` as `${number}%`,
                ...
                <View style={styles.compactTrack}>
                  <View
                    style={[
                      styles.compactFill,
                      {
                        width: `${em.intensityAfter}%` as `${number}%`,
...
function LeftColumn({ page, record }: { page: number; record: RecordType }) {
  ...
  switch (page) {
    case 0:
      return (
        <ColPanel title="Sytuacja">
          <ColText text={record.situation} />
        </ColPanel>
      );
    case 1:
      return (
        <ColPanel title="Emocje">
          <CompactEmotionBars emotions={record.emotions} />
        </ColPanel>
      );
    case 2:
      return (
        <ColPanel title="Myśli automatyczne">
          <ColText text={record.automaticThoughts} />
        </ColPanel>
      );
    case 3:
      return (
...
function RightColumn({ page, record }: { page: number; record: RecordType }) {
  ...
  switch (page) {
    ...
    case 3:
      return (
        <ColPanel title="Myśl alternatywna">
          <ColText text={record.alternativeThought} />
        </ColPanel>
      );
    default:
      return null;
