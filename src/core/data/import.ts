...
export async function importData(
  ...
  for (const r of file.thoughtRecords) {
    const row = await db.getFirstAsync<{ id: string }>('SELECT id FROM tool_entries WHERE id = ?', [r.id as string]);
    if (row) existingIds.add(r.id as string);
  }
  for (const e of file.behavioralExperiments) {
    const row = await db.getFirstAsync<{ id: string }>('SELECT id FROM tool_entries WHERE id = ?', [e.id as string]);
  ...
  await db.withTransactionAsync(async () => {
    for (const r of file.thoughtRecords) {
      ...
      await db.runAsync(
        ...
        [id, createdAt, now, r.is_complete ? 1 : 0, typeof r.current_step === 'number' ? r.current_step : 1]
      );
      await db.runAsync(
        ...
        [
          ...
          Array.isArray(r.emotions) ? JSON.stringify(r.emotions) : typeof r.emotions === 'string' ? r.emotions : '[]',
    ...
    for (const e of file.behavioralExperiments) {
      ...
      await db.runAsync(
        ...
        [id, createdAt, now, e.is_complete ? 1 : 0, typeof e.current_step === 'number' ? e.current_step : 1]
      );
      await db.runAsync(
  ...
  if (file.settings && typeof file.settings === 'object') {
    ...
    if (s.fontSize === 'sm' || s.fontSize === 'md' || s.fontSize === 'lg') patch.fontSize = s.fontSize;
